...
CCO_model = XGBRegressor(n_estimators=1000, learning_rate=0.05, n_jobs=4)
CCO_model.fit(X_train, y_train,
             early_stopping_rounds=5,
             eval_set=[(X_valid, y_valid)],
             verbose=False)

predictions_new = CCO_model.predict(X_valid)
print("Mean Absolute Error: " + str(mean_absolute_error(predictions_new, y_valid)))

#%%save
with open('CCO_model.pkl','wb') as f:
    pickle.dump(CCO_model,f)
